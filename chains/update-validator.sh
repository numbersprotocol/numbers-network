...
update_subnet_evm() {
    ...
    cp "subnet-evm-${SUBNET_EVM_VERSION}/subnet-evm" "${HOME}/.avalanchego/plugins/${VM_ID}"
    sha256sum "subnet-evm-${SUBNET_EVM_VERSION}/subnet-evm" "${HOME}/.avalanchego/plugins/${VM_ID}"
}

show_validator_files() {
    echo "Step: show_validator_files"
    tree "avalanchego-v${AVALANCHEGO_VERSION}"
    tree "${HOME}/.avalanchego/plugins/"
}
